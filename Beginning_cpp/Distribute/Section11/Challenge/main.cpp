...
void target_find(const vector<int> &v);
...
void display_menu() {
    ...
    cout<<"F - Find a number"<<endl; // 목록에서 숫자가 몇번 나왔는지 검색
...
char get_selection() {
    char selection;
    cin>>selection;
    return toupper(selection); // 대문자로 변환
}
...
void handle_largest(const vector<int> &v) {
    if(v.size() == 0)
        ...
        cout<<"The largest element in the list is "<<get_largest(v);
}
...
void handle_clear(vector<int> v) {
    v.clear();
    cout<<"Clear!!";
}
